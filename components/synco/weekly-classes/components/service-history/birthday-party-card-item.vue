<template>
  <div class="d-flex flex-column px-3">
    <span class="h5 text-semibold m-0">Package</span>
    <span>Gold</span>
  </div>
  <hr />
  <div class="d-flex flex-column px-3">
    <span class="h5 text-semibold m-0">Monthly price</span>
    <span>£39.99</span>
  </div>
  <hr />
  <div class="d-flex flex-column px-3">
    <span class="h5 text-semibold m-0">Stripe Transaction ID</span>
    <span>XHDJDHL3314</span>
  </div>
  <hr />
  <div class="d-flex flex-column px-3">
    <span class="h5 text-semibold m-0">Date of booking</span>
    <span>Nov 18 2021, 17:00</span>
  </div>
  <hr />
  <div class="d-flex flex-column px-3">
    <span class="h5 text-semibold m-0">Date of Party</span>
    <span>Nov 18 2021, 17:00</span>
  </div>
  <hr />
  <div class="d-flex flex-column px-3">
    <span class="h5 text-semibold m-0">Coach</span>
    <span>Ethan Bond-Vaughan</span>
  </div>
  <hr />
  <div class="d-flex flex-column px-3">
    <span class="h5 text-semibold m-0">Booking Source</span>
    <span>Ben Marcus</span>
  </div>
  <hr />
</template>
